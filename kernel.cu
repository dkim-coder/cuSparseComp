...
int cusLtMatmul(T* hA, T* hB, T* hC, T* hA_pruned, const int m, const int n, const int k, const int device_num = 0)
{
    ...
    CHECK_CUSPARSE(cusparseLtSpMMAPrune(&handle, &matmul, dA, dA, CUSPARSELT_PRUNE_SPMMA_TILE, NULL))
    CHECK_CUSPARSE(cusparseLtSpMMAPruneCheck(&handle, &matmul, dA, d_valid, NULL))
    int is_valid;
    CHECK_CUDA(cudaMemcpy(&is_valid, d_valid, sizeof(*d_valid), cudaMemcpyDeviceToHost))
    if (is_valid != 0) {
        std::cout << "Sparse matrix(A) has been pruned in a wrong way. cusparseLtMatmul will not provide correct results" << std::endl;
        return EXIT_FAILURE;
    }
    ...
    CHECK_CUSPARSE(cusparseLtSpMMACompress(&handle, &plan, dA, dA_compressed, NULL))
